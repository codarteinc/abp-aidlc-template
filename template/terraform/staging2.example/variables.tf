# Variables for the staging2 example root.
#
# Mirrors `terraform/staging/variables.tf` exactly (same operator-facing
# knobs, same validation rules). This is the canonical shape for any
# additional env beyond staging — copy this directory (preferably via
# `scripts/new-terraform-env.sh`) and rewrite `env_name` + the HCP
# workspace name.
...
variable "cloudflare_api_token" {
  description = "Cloudflare API token with Zone:Edit on the env's zone. Only required when `dns_provider = \"cloudflare\"`. Pass via TF_VAR_cloudflare_api_token env var; never commit."
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "ssh_allowed_cidrs" {
  description = "List of CIDR blocks allowed to SSH (port 22) into the VM. Example: [\"203.0.113.7/32\", \"198.51.100.42/32\"]."
  type        = list(string)
...
variable "server_type" {
  description = "Hetzner Cloud server type. cx23 is the smallest current x86 box; bump to cx33 once the stack starts swapping under real load."
  type        = string
...
variable "cloudflare_zone_id" {
  description = "Cloudflare zone ID (NOT the domain name) where the env's records will be created. Only consumed when `dns_provider = \"cloudflare\"`."
  type        = string
  default     = ""
}

variable "app_hostname" {
  description = "Fully-qualified hostname for the SPA edge. When empty and dns_provider=\"none\", the module falls back to an sslip.io hostname derived from the primary IPv4."
  type        = string
  default     = ""
}

variable "api_hostname" {
  description = "Fully-qualified hostname for the API edge. When empty and dns_provider=\"none\", the module falls back to an sslip.io hostname derived from the primary IPv4."
  type        = string
  default     = ""
}
